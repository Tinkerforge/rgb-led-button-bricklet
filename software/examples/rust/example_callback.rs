...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    // Create receiver for button state changed events.
    let button_state_changed_receiver = rlb.get_button_state_changed_receiver();

    // Spawn thread to handle received events. This thread ends when the `rlb` object
